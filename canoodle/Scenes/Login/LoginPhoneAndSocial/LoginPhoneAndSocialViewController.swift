//
//  LoginPhoneAndSocialViewController.swift
//  WhiteLabelApp
//
...
protocol LoginPhoneAndSocialDisplayLogic: class {
    ...
    /// Did Receive Social LoginAPI Response
    ///
    /// - Parameters:
    ///   - Response: API Response
    ///   - message: API Message
...
/// This class is used for login action of user. Login can be done using phone number or using social accounts.
class LoginPhoneAndSocialViewController: BaseViewController {
    /// Interactor for API Call
    var interactor: LoginPhoneAndSocialBusinessLogic?
    /// Router for navigation between the screens
    var router: (NSObjectProtocol & LoginPhoneAndSocialRoutingLogic & LoginPhoneAndSocialDataPassing)?
    ...
    /// Instance
    ///
    /// - Returns: LoginPhoneAndSocialViewController
    class func instance() -> LoginPhoneAndSocialViewController? {
        return StoryBoard.LoginPhoneAndSocial.board.instantiateViewController(withIdentifier: AppClass.LoginPhoneAndSocialVC.rawValue) as? LoginPhoneAndSocialViewController
    }
    ...
    private func setup() {
        ...
        let interactor = LoginPhoneAndSocialInteractor()
        let presenter = LoginPhoneAndSocialPresenter()
        let router = LoginPhoneAndSocialRouter()
    ...
    func validateFields() {
        ...
        do {
            ...
            self.interactor?.loginWithPhone(request: LoginPhoneAndSocialModel.Request(phone: phoneNumber, password: password))
            ...
            GraphRequest(graphPath: "me", parameters: ["fields": "id, name, first_name, last_name, picture.type(large), email"]).start(completionHandler: { (_, result, error) -> Void in
                if (error == nil) {
                    if let aResult = result as? [String : AnyObject] {
                        ...
                        if let socialID = (aResult["id"] as? String) {
                            self.callSocialLoginApi(type: self.socialLoginType, id: socialID)
                        }
                    }
                }
            })
    ...
    func callSocialLoginApi(type: String, id: String) {
        self.interactor?.socialLogin(request: LoginPhoneAndSocialModel.SocialRequest(type: type, id: id))
    }
    
    // MARK: WLButton Actions
    
    /// Phone Number Field formatting while text editing
...
extension LoginPhoneAndSocialViewController: GIDSignInDelegate {
...
extension LoginPhoneAndSocialViewController : ASAuthorizationControllerDelegate {
    @available(iOS 13.0, *)
...
extension LoginPhoneAndSocialViewController : ASAuthorizationControllerPresentationContextProviding {
    @available(iOS 13.0, *)
...
extension LoginPhoneAndSocialViewController: LoginPhoneAndSocialDisplayLogic {
    // login with Phone number api response
    func didReceiveLoginResponse(response: Login.ViewModel?, message: String, success: String) {
        if success == "1" {
