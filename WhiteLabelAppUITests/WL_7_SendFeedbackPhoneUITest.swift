...
class WL_7_SendFeedbackPhoneUITest: WhiteLabelAppUITests {
    ...
    func test1SendFeedback() {
        ...
        wait(for: delay)
        snapshot("7.0.1SendFeedScreen",timeWaitingForIdle: 3)
        
        // Empty TextView
        let sendButton = elementsQuery.buttons["Send"]
        sendButton.tap()
        snapshot("7.0.2EmptyText",timeWaitingForIdle: 3)
        ...
        sendButton.tap()
        snapshot("7.0.3Success",timeWaitingForIdle: 10)
        wait(for: apiDelay)
        wait(for: delay)
